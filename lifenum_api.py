...
"""
    ...
    # 建立 user prompt
    if user_purpose:
        user_prompt = f"生日：{birthdate}\n{extra_info}計算結果數字：{display_number}\n\n【使用者的問題】\n{user_purpose}\n\n請先檢查問題是否違反【內容限制】規則。若違反，請直接拒絕。若無違反，請根據計算結果提供完整詳細的生命靈數解析。**【極度重要】在正式回覆的第一段，請務必先明確告訴使用者他的數字是多少（例如：「你的靈魂數/業力數/生命靈數是 X」）**，接著再針對他的問題給予相應的指引與建議。回應必須包含指定的稱呼開頭，且內容詳盡。"
    else:
        user_prompt = f"生日：{birthdate}\n{extra_info}計算結果數字：{display_number}\n\n請提供完整詳細的生命靈數解析，包含性格底色、優勢、人生方向等所有相關內容。**【極度重要】在正式回覆的第一段，請務必先明確告訴使用者他的數字是多少（例如：「你的靈魂數/人格數/生命靈數是 X」）**，然後再開始後續的深度解析內容。回應必須包含指定的稱呼開頭，且提供至少300字以上的深度解析。"
