    ...
    try:
        ...
        if number == 0:
            return (
                ...
                "[特殊情況]：使用者的業力數為「無」，代表「無特定業力債」。\n"
                "[意義]：象徵靈魂是自由的，此生沒有前世遺留的重大未竟課題，這是一個正向的指標。\n"
                "【重要指示】：若使用者有提出「具體問題」，請直接針對其問題，結合「無特定業力」的特質（自由、無包袱、可塑性高）給予解答與建議即可，不要重複輸出下方的固定格式。\n"
        ...
        return (
            ...
            f"[意義]：{desc}\n"
            "【重要指示】：若使用者有提出「具體問題」，請直接針對其問題，結合上述的業力意義給予解答與轉化建議即可，不要重複輸出下方的固定格式。\n"
            "若使用者「沒有」具體提問（初次計算），請根據使用者的業力數，參考上述意義進行深度解析，並給出：\n"
            f"- 你的業力數：{number}\n"
            "- 此生帶來的因果課題（3-5 點）\n"
            "- 如果在生活中遇到阻礙，通常是...（舉例）\n"
            "- 如何償還或轉化這份業力（具體行動）\n"
