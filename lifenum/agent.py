...
class ConversationState(Enum):
    ...
    # 通用狀態（適用於所有模組）
    CONTINUE_SELECTION = (
        "continue_selection"  # 等待繼續選項（繼續問問題、其他生命靈數、離開）
    )
...
class ConversationSession:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "maturity_number": self.maturity_number,
            "challenge_number": self.challenge_number,
            "karma_number": self.karma_number,
        }
...
class LifeNumberAgent:
    ...
    def calculate_age(self, birthdate: str) -> int:
        ...
        try:
            # 處理不同的日期格式
            birthdate = (
                birthdate.replace("年", "/").replace("月", "/").replace("日", "")
            )
    ...
    def generate_category_buttons_message(self, tone: str = "guan_yu") -> str:
        ...
        if tone == "guan_yu":
            ...
            return (
                "這裡有四種力量面向可供探索。選擇一個您需要的力量，讓我為您指引方向。"
            )
        elif tone == "gabriel":
    ...
    def generate_question_prompt(self, category: str, tone: str = "guan_yu") -> str:
        ...
        if tone == "guan_yu":
            ...
            return (
                f"您選擇了「{category_name}」的啟示。請告訴我您最渴望接收的真理是什麼？"
            )
        elif tone == "raphael":
    ...
    def generate_continue_prompt(self, tone: str = "guan_yu") -> str:
        ...
        if tone == "guan_yu":
            return (
                "你可以繼續詢問，或選擇探索其他生命靈數，也可以離開。我隨時為你解惑。"
            )
        elif tone == "michael":
    ...
    def generate_conversation_summary(self, session, tone: str = "guan_yu") -> str:
        ...
        if session.user_name and session.birthdate and session.core_number:
            summary_points.append(
                f"為 {session.user_name} 解析了生命靈數 {session.core_number}"
            )
        ...
        if tone == "guan_yu":
            ...
            if summary_points:
                summary += "我們一起探索了：\n" + "\n".join(
                    [f"⚔️ {point}" for point in summary_points]
                )
                summary += (
            ...
            if summary_points:
                ...
                summary += (
                    "雖然時間短暫，但我的守護將永遠與您同在。勇敢前行，光明與您同在。"
                )
        elif tone == "gabriel":
            ...
            if summary_points:
                ...
                summary += (
                    "雖然相遇短暫，但愛的能量已在我們之間流動。願您帶著滿滿的愛前行。"
                )
        elif tone == "uriel":
            ...
            if summary_points:
                ...
                summary += (
                    "每一次的相遇都是轉化的開始。願您帶著寬恕的心，迎接生命的新篇章。"
                )
        elif tone == "jophiel":
            ...
            if summary_points:
                ...
                summary += f"\n\n願這些美麗的智慧讓您的生命如花般綻放，光彩照人。您的光芒將持續綻放。"
            else:
            ...
            if summary_points:
                ...
                summary += (
                    "每一次的相遇都是愛的連結。願您帶著滿滿的愛，創造更美好的關係。"
                )
        elif tone == "metatron":
    ...
    def generate_year_question_prompt(self, tone: str = "guan_yu") -> str:
        ...
        if tone == "guan_yu":
            return (
                "你有什麼疑惑？關於流年運勢或人生規劃的事，都可以詢問，我會為你分析。"
            )
        elif tone == "michael":
    ...
    def generate_soul_question_prompt(self, tone: str = "guan_yu") -> str:
        ...
        if tone == "guan_yu":
            return (
                "你有什麼疑惑？關於內心渴望或精神追求的事，都可以詢問，我會為你分析。"
            )
        elif tone == "michael":
    ...
    def generate_personality_question_prompt(self, tone: str = "guan_yu") -> str:
        ...
        if tone == "guan_yu":
            return (
                "你有什麼疑惑？關於外在人格或社交印象的事，都可以詢問，我會為你分析。"
            )
        elif tone == "michael":
    ...
    def generate_expression_question_prompt(self, tone: str = "guan_yu") -> str:
        ...
        if tone == "guan_yu":
            return (
                "你有什麼疑惑？關於表達風格或溝通方式的事，都可以詢問，我會為你分析。"
            )
        elif tone == "michael":
    ...
    def generate_challenge_question_prompt(self, tone: str = "guan_yu") -> str:
        ...
        if tone == "guan_yu":
            return (
                "你有什麼疑惑？關於此生需要克服的課題或限制，都可以詢問，我會為你分析。"
            )
        elif tone == "michael":
...
"""
        ...
        error_note = (
            "6. 如果缺少姓名、性別、生日或英文名字，都需要在 error_message 中說明"
            if require_english_name
            else "6. 如果缺少姓名、性別或生日，都需要在 error_message 中說明（英文名字為選填）"
        )
...
9. challenge - 挑戰數：適合想了解此生需要克服的困難、人生課題、限制與突破點的人
10. karma - 業力數：適合想了解前世未竟課題、今生因果、深層恐懼與轉化的人
...
"""
        ...
        try:
            ...
            return (
                "core",
                "根據您的描述，我建議從核心生命靈數開始，這能幫助您更全面地了解自己的天賦與人生方向。",
            )
    ...
    def generate_rejection_response(
        ...
        if tone == "friendly":
            ...
            return (
                f"好的，{name}。那麼請您再詳述您的需求，在下將重新為您推薦合適之解析。"
            )

        # 付費版語氣
        elif tone == "guan_yu":
            return f"很好，{name}。{you}可以直接說出與{you}的緣分：「生日數」、「核心生命靈數」、「九宮格」、「流年數」、「靈魂數」、「人格數」、「表達數」、「成熟數」、「挑戰數」或「業力數」，我會為{you}分析。"
        elif tone == "michael":
            return f"了解，{name}。請直接告訴我{you}想探索的力量：「生日數」、「核心生命靈數」、「九宮格」、「流年數」、「靈魂數」、「人格數」、「表達數」、「成熟數」、「挑戰數」或「業力數」，我將為{you}指引方向。"
        elif tone == "gabriel":
            return f"明白了，{name}。請直接告訴我{you}想接收的啟示：「生日數」、「核心生命靈數」、「九宮格」、「流年數」、「靈魂數」、「人格數」、「表達數」、「成熟數」、「挑戰數」或「業力數」，我將傳達相應的訊息。"
        elif tone == "raphael":
            return f"當然可以，親愛的{name}。請告訴我{you}想療癒的方面：「生日數」、「核心生命靈數」、「九宮格」、「流年數」、「靈魂數」、「人格數」、「表達數」、「成熟數」、「挑戰數」或「業力數」，讓我為{you}帶來所需的指引。"
        elif tone == "uriel":
            return f"理解。{name}，請直接說出{you}渴望獲得智慧的領域：「生日數」、「核心生命靈數」、「九宮格」、「流年數」、「靈魂數」、「人格數」、「表達數」、「成熟數」、「挑戰數」或「業力數」，我將點亮相應的洞察。"
        elif tone == "zadkiel":
            return f"很好，{name}。請告訴我{you}想轉化的面向：「生日數」、「核心生命靈數」、「九宮格」、「流年數」、「靈魂數」、「人格數」、「表達數」、「成熟數」、「挑戰數」或「業力數」，讓我們一起走向理解。"
        elif tone == "jophiel":
            return f"好的呀，{name}。請告訴我{you}想發掘的美好：「生日數」、「核心生命靈數」、「九宮格」、「流年數」、「靈魂數」、「人格數」、「表達數」、「成熟數」、「挑戰數」或「業力數」，讓我們一起探索{you}的光芒。"
        elif tone == "chamuel":
            return f"當然沒問題，親愛的{name}。請告訴我{you}想了解的關係層面：「生日數」、「核心生命靈數」、「九宮格」、「流年數」、「靈魂數」、「人格數」、「表達數」、「成熟數」、「挑戰數」或「業力數」，讓愛為{you}指引。"
        elif tone == "metatron":
            return f"收到，{name}。請直接指定{you}需要分析的模組：「生日數」、「核心生命靈數」、「九宮格」、「流年數」、「靈魂數」、「人格數」、「表達數」、「成熟數」、「挑戰數」或「業力數」，系統將執行相應程序。"
        elif tone == "ariel":
            return f"好的，{name}。請告訴我{you}想滋養的生命面向：「生日數」、「核心生命靈數」、「九宮格」、「流年數」、「靈魂數」、「人格數」、「表達數」、「成熟數」、「挑戰數」或「業力數」，讓大地的智慧為{you}指引。"
    ...
    def generate_confirmation_retry(
        ...
        # 付費版語氣
            ...
            return (
                f"這個訊息我沒有接收清楚。請回覆「好」或「不要」，我將傳達相應的指引。"
            )
        elif tone == "raphael":
